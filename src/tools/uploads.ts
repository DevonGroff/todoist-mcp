/**
 * File uploads + task attachments (ported from Devon's Apr-2026 upstream commit into the v2
 * registry). Upload a local file to Todoist's CDN, then reference it from a comment's attachment.
 * `attachFileToTask` is the one-call convenience (upload → comment-with-attachment).
 */
