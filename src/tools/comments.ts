...
export function formatCommentWithPrefix(
  content: string,
...
export function parseCommentPrefix(content: string): {
  prefix: CommentPrefix | null;
  content: string;
} {
...
export async function listComments(params: {
  ...
  try {
    ...
    const results = await client.getAllPaginated<TodoistComment>(
      "/comments",
...
export async function createComment(
  params: CreateCommentParams & { prefix?: CommentPrefix },
): Promise<ToolResponse<TodoistComment>> {
  try {
    if (!params.task_id && !params.project_id) {
      return createResponse(false, undefined, {
    ...
    const comment = await client.post<TodoistComment>(
      "/comments",
      requestParams,
    );
    return createResponse(true, comment);
