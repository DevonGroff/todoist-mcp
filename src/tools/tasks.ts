...
): Promise<ToolResponse<TodoistTask[] | PaginatedResult<TodoistTask>>> {
  try {
    const client = getApiClient();
    const manualPagination =
      params.cursor !== undefined || params.limit !== undefined;
    ...
    if (params.filter) {
      ...
      if (manualPagination) {
        ...
        const response = await client.get<PaginatedResponse<TodoistTask>>(
          "/tasks/filter",
          queryParams,
        );
      ...
      const results = await client.getAllPaginated<TodoistTask>(
        "/tasks/filter",
        queryParams,
      );
...
export async function getTask(
  taskId: string,
): Promise<ToolResponse<TodoistTask>> {
  try {
    const client = getApiClient();
...
export async function completeTask(
  taskId: string,
): Promise<ToolResponse<{ completed: boolean }>> {
  try {
    const client = getApiClient();
...
export async function reopenTask(
  taskId: string,
): Promise<ToolResponse<{ reopened: boolean }>> {
  try {
    const client = getApiClient();
...
export async function searchTasks(
  query: string,
  params: Omit<ListTasksParams, "filter"> = {},
): Promise<ToolResponse<TodoistTask[]>> {
  try {
    const client = getApiClient();
    const queryParams: Record<string, unknown> = {
